...
$outputFile = "e:\Nouveau dossier (12)\awards_labels.txt"

try {
    $conn.Open($connectionString)
    
    $tables = @("mention", "sanction", "decicons")
    ...
    foreach ($table in $tables) {
        ...
        try {
            ...
            $rs.Close()
        } catch { "Table $table not readable." | Out-File -FilePath $outputFile -Append -Encoding utf8 }
